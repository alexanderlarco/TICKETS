...
perfilCtl.mostrar = async (req, res) => {
    ...
    const usuarios = await sql.query('select * from usuarios where idUsuarios = ?', [id])
    res.render('perfil/perfil', { usuarios});
}

perfilCtl.mandar = async (req, res) => {
    const id = req.user.idUsuarios
    const { Nombres, Apellidos, username, password, email, edad, celular, si, no } = req.body
    const nuevoEnvio = {
        Nombres,
        Apellidos,
        username,
    ...
    await sql.query('update usuarios set ? where idUsuarios= ?', [nuevoEnvio, id])
    if (no == 'no') {
