...
ProductoEntradaCtrl.renderEntrada = async (req, res) => {
	...
	const listaCategoria = await sql.query(
		"SELECT * FROM categorias ORDER BY categoria ASC"
	);
	const idProductoEntrada = await sql.query("SELECT * FROM idmaximo");
	const listaUnidad = await sql.query(
		"SELECT * FROM unidadMedidas ORDER BY unidadMedida ASC"
	);
...
ProductoEntradaCtrl.renderProductos = async (req, res) => {
	const id = req.params.id;
	const DatosProducto = await sql.query(
		"SELECT e.*, u.unidadMedida FROM detalle_productosentrada e JOIN unidadmedidas u ON  e.unidadMedidaIdUnidadMedidas = u.idUnidadMedidas  WHERE e.tiendaIdTiendas = ?",
		[id]
	);
...
ProductoEntradaCtrl.EditarEntrada = async (req, res) => {
	...
	await orm.entredaProductos
		.findOne({ where: { idProductoEntradas: id } })
