...
PedidosCtrl.mostrar = async (req, res) => {
	...
	const listaProductos = await sql.query(
		"SELECT * FROM productoentradas pro JOIN provedores p ON provedoreIdProvedores = idProvedores WHERE idProvedores =?",
		[id]
	);
	const listapedidos = await sql.query(
		"SELECT * FROM pedidos ORDER BY idPedidos ASC"
	);
	const idPedidos = await sql.query("SELECT * FROM idmaximoPedidos");
	res.render("pedidos/pedidosAgregar", {
		listaProveedor,
		listaProductos,
		listapedidos,
		idPedidos,
...
PedidosCtrl.lista = async (req, res) => {
	const ids = req.params.id;
	const pedidoslista = await sql.query(
		"SELECT * FROM detalle_pedidos  WHERE detalleSubRolTiendaIdDetalleSubRolTienda = ?",
		[ids]
	);
	const traer = await sql.query(
		"SELECT DISTINCT nombre FROM detalle_pedidos"
	);
