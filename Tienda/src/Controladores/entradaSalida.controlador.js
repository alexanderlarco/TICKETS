...
entradaSalida.mostrarEntrada = async (req, res) => {
	...
	const listaTienda = await sql.query(
		"SELECT nombreNegocio FROM tiendas WHERE detalleSubRolTiendaIdDetalleSubRolTienda = ?",
		[ids]
	);
	const listaProveedor = await sql.query(
		"SELECT * FROM provedores WHERE detalleSubRolTiendaIdDetalleSubRolTienda = ?",
		[ids]
	);
	const listaProductos = await sql.query(
		"SELECT DISTINCT v.productoCantidad, p.*, u.unidadMedida FROM productoentradas p JOIN unidadmedidas u ON u.idUnidadMedidas = p.unidadMedidaIdUnidadMedidas JOIN productos v ON p.idProductoEntradas = v.productoEntradaIdProductoEntradas WHERE p.tiendaIdTiendas = ?",
		[ids]
	);
	res.render("EntradasSalidas/entradas/entadaAgregar", {
		listaTienda,
		listaProveedor,
		listaProductos,
		entrasLista,
...
entradaSalida.mandarEntrada = async (req, res) => {
	...
	const {
		...
		cantidadRestante,
		idRegistroEntradas,
		productoEntradaIdProductoEntradas,
		provedoreIdProvedores,
	} = req.body;
	...
	for (let i = 0; i < productoEntradaIdProductoEntradas.length; i++) {
		await sql.query(
			"INSERT INTO detalleregistroentradas(entraCantidad,cantidadRestante,creacionRegistroEntradas,registroEntradaIdRegistroEntradas,productoEntradaIdProductoEntradas) VALUES (?,?,?,?,?)",
			[
				entraCantidad[i],
				cantidadRestante[i],
				fecha,
				idRegistroEntradas,
				productoEntradaIdProductoEntradas[i],
			]
		);
	}
...
entradaSalida.detallelistaEntrada = async (req, res) => {
	...
	const lista = await sql.query(
		"SELECT * FROM detatalleentradaproductos WHERE registroEntradaIdRegistroEntradas = ?",
		[id]
	);
...
entradaSalida.mostrarSalida = async (req, res) => {
	...
	const listaTienda = await sql.query(
		"SELECT nombreNegocio FROM tiendas WHERE IdTiendas = ?",
		[ids]
	);
	const listaClientes = await sql.query(
		"SELECT * FROM Clientes WHERE idClientes = ?",
		[ids]
	);
	const listaProductos = await sql.query(
		"SELECT * FROM listaSalidas  WHERE listaProductoIdListaProductos = ?",
		[id]
	);
	res.render("EntradasSalidas/salidas/salidasAgregar", {
		salidaLista,
		listaTienda,
		listaClientes,
		listaProductos,
...
entradaSalida.MandarSalida = async (req, res) => {
	...
	for (let i = 0; i < productoIdProductos.length; i++) {
		await sql.query(
			"INSERT INTO detalleregistrosalidas(Precio,ventaCantidad,salidaCantidad,cantidadRestante,creacionDetalleRegistroSalidas,productoIdProductos,registroSalidaIdRegistroSalidas) VALUES (?,?,?,?,?,?,?)",
			[
				Precio[i],
				ventaCantidad[i],
				salidaCantidad,
...
entradaSalida.detallelistaSalidas = async (req, res) => {
	const id = req.params.id;
	const lista = await sql.query(
		"SELECT * FROM salidaProductos WHERE registroSalidaIdRegistroSalidas = ?",
		[id]
	);
