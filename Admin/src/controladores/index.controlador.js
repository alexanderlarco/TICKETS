...
indexCtrl.mandar = async (req, res) => {
    ...
    if (categorias.length == 0) {
        ...
        if (categoria === undefined) {
            ...
            await pool.query("CREATE VIEW ProductosLista AS SELECT p.*, e.NombreProducto, d.unidadVenta, d.cantidadVenta FROM productos p JOIN productoentradas e ON e.idProductoEntradas = p.productoEntradaIdProductoEntradas JOIN detallecategorias d ON d.idDetalleCategorias = p.detalleCategoriaIdDetalleCategorias")
            ...
            await pool.query("CREATE VIEW detatalleEntradaProductos AS SELECT d.*, p.NombreProducto, p.precioUnidad FROM detalleregistroentradas d JOIN productoentradas p ON d.productoEntradaIdProductoEntradas = p.idProductoEntradas")
            ...
            await pool.query("CREATE VIEW detalle_productosentrada as select u.unidadmedida, p.*, d.cantidadMedida from productoentradas p join unidadmedidas u on u.idunidadmedidas = p.unidadMedidaIdUnidadMedidas join detalleunidadmedidas d on u.idunidadmedidas = d.unidadMedidaIdUnidadMedidas")
